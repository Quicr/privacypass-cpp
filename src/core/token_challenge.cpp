...
Result<TokenChallenge> TokenChallenge::deserialize(ByteView data) {
    ...
    // redemption_context (check remaining data to determine if present)
    // If there's more data after origin_info length, it has a context
    size_t remaining = reader.remaining();
    if (remaining > 2) {
        // We need to peek at the origin_info length to determine structure
        // This is a simplified parser - real implementation needs more careful handling

        // Try reading as having redemption_context
